...
- Target database: ipl_rcb_players2.
- Table: ipl_rcb_players2.
- Columns: players_name (text), players_role (text), players_age (int), players_salary (float).
...
A: SELECT COUNT(*) FROM ipl_rcb_players2
Q: Show all Allrounder
A: SELECT * FROM ipl_rcb_players2 WHERE players_role = 'Allrounder'

Q: Who earns more than 600000?
A: SELECT * FROM ipl_rcb_players2 WHERE players_salary > 600000

Q: Who earns the highest salary?
A: SELECT * FROM ipl_rcb_players2 ORDER BY players_salary DESC LIMIT 1
...
st.title("Gemini SQL Assistant (RCB Players DB)")
...
if st.button("Run"):
    if question.strip() == "":
        ...
        # Run SQL query
        result, columns = read_sql_query(sql_query, "ipl_rcb_players2.db")
